<!-- Top Bar -->
<header class="glass h-20 shadow-lg flex items-center justify-between px-4 md:px-6 lg:px-8 relative z-30 rounded-[1.5rem] mb-6">
    <div class="flex items-center">
        <button id="sidebar-toggle"
            class="md:hidden text-gray-600 hover:text-primary mr-4 p-2 hover:bg-purple-50 rounded-2xl transition-all">
            <i class="fas fa-bars text-xl"></i>
        </button>
        <div>
            <h2 class="text-lg md:text-2xl font-bold text-gray-900 font-montserrat">
                @yield('title', 'Dashboard')
            </h2>
            <p class="text-xs text-gray-500 hidden sm:block tracking-[0.18em] uppercase">{{ now()->format('l, d F Y') }}</p>
        </div>
    </div>
    ...
    <div class="flex items-center space-x-2 md:space-x-4">
        <!-- Search -->
        <button
            class="hidden md:flex items-center px-4 py-2.5 border border-[#eadfce] bg-white/80 hover:bg-white rounded-full transition-colors">
            <i class="fas fa-search text-gray-600 mr-2"></i>
            <span class="text-sm text-gray-600">Navigation admin</span>
        </button>
        ...
        <div x-data="{ open: false }" class="relative">
            ...
            <button @click="open = !open"
                class="relative flex items-center justify-center p-2.5 rounded-full hover:bg-[#f6efe5] transition">
        ...
        <a href="{{ route('admin.profile') }}"
            class="hidden sm:block text-gray-600 hover:text-primary p-2.5 hover:bg-purple-50 rounded-full transition-all">
        ...
        <form method="POST" action="{{ route('admin.logout') }}" class="inline">
            @csrf
            <button type="submit"
                class="flex items-center space-x-2 px-3 md:px-4 py-2.5 rounded-full bg-[#2a2038] text-white transition-all shadow-md hover:bg-black hover:shadow-lg">

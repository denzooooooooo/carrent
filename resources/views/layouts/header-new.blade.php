@php
    ...
    $mainNavigation = [
        ['label' => 'Accueil', 'route' => 'home', 'patterns' => ['home'], 'icon' => 'fa-house'],
        ['label' => 'Événements', 'route' => 'events', 'patterns' => ['events', 'events.*'], 'icon' => 'fa-ticket'],
        ['label' => 'Packages', 'route' => 'packages', 'patterns' => ['packages', 'packages.*'], 'icon' => 'fa-suitcase-rolling'],
        ['label' => 'Location', 'route' => 'location', 'patterns' => ['location', 'location.*'], 'icon' => 'fa-car-side'],
        ['label' => 'Vols', 'route' => 'flights.index', 'patterns' => ['flights.*'], 'icon' => 'fa-plane-up'],
    ];
...
>
    <header class="px-3 pt-3 lg:px-4 lg:pt-4">
        <div class="cp-shell">
            <div class="cp-glass rounded-[1.75rem] px-4 py-3 lg:px-5 lg:py-4">
                <div class="flex items-center gap-3">
                    <a href="{{ route('home') }}" class="flex min-w-0 flex-1 items-center gap-3 lg:flex-none">
                        <span class="inline-flex h-11 w-11 flex-none items-center justify-center overflow-hidden rounded-2xl border border-white/50 bg-white shadow-md">
                            <img src="{{ asset('logos/logo2.jpg') }}" alt="{{ $companyName }}" class="h-full w-full object-cover">
                        </span>
                        <span class="min-w-0">
                            <span class="block truncate text-sm font-black tracking-[0.08em] text-[color:var(--cp-plum-950)] sm:text-base">{{ strtoupper($companyName) }}</span>
                            <span class="block truncate text-[10px] font-semibold uppercase tracking-[0.22em] text-[color:var(--cp-ink-muted)]">Travel, Events, Concierge</span>
                        </span>
                    </a>
                    ...
                    <nav class="hidden flex-1 items-center justify-center gap-2 lg:flex">
                        @foreach($mainNavigation as $item)
                            ...
                            <a
                                ...
                                @class([
                                    'cp-nav-link !px-4 !py-2.5 !text-sm',
                                    'is-active' => $active,
                                ])
                    ...
                    <div class="hidden items-center gap-2 lg:flex">
                        <a href="{{ $mobileLink }}" class="cp-icon-button" aria-label="Appeler" title="Appeler">
                            <i class="fa-solid fa-phone text-sm"></i>
                        </a>
                        <a href="{{ $whatsAppUrl }}" target="_blank" rel="noopener noreferrer" class="cp-icon-button" aria-label="WhatsApp" title="WhatsApp">
                        ...
                        <div x-data="{ open: false }" class="relative hidden xl:block">
                            <button
                                type="button"
                                @click="open = !open"
                                class="cp-secondary-button !px-4 !py-2.5 text-sm"
                            ...
                            <div
                                ...
                                x-transition
                                class="absolute right-0 mt-3 w-72 rounded-[1.5rem] border border-[color:var(--cp-border)] bg-white p-4 shadow-2xl"
                            >
                                <div>
                                    <p class="text-xs font-black uppercase tracking-[0.18em] text-[color:var(--cp-ink-muted)]">Langue</p>
                                    <div class="mt-3 flex gap-2">
                                        <button type="button" onclick="changeLanguage('fr')" class="cp-secondary-button !px-4 !py-2.5 text-sm">FR</button>
                                        <button type="button" onclick="changeLanguage('en')" class="cp-secondary-button !px-4 !py-2.5 text-sm">EN</button>
                                    </div>
                                </div>

                                <div class="mt-4">
                                    <p class="text-xs font-black uppercase tracking-[0.18em] text-[color:var(--cp-ink-muted)]">Devise</p>
                                    <div class="mt-3 flex flex-wrap gap-2">
                                        @foreach(['XOF', 'EUR', 'USD', 'GBP'] as $currency)
                                            <button type="button" onclick="changeCurrency('{{ $currency }}')" class="cp-secondary-button !px-4 !py-2.5 text-sm">
                                                {{ $currency }}
                                            </button>
                                        @endforeach
                                    </div>
                        ...
                        <a href="{{ route('contact') }}" class="cp-primary-button !px-4 !py-2.5 text-sm xl:!px-5">
                            <i class="fa-solid fa-headset text-sm"></i>
                            <span class="hidden xl:inline">Conseiller</span>
                            <span class="xl:hidden">Aide</span>
                        </a>
                        ...
                        @else
                            <div x-data="{ open: false }" class="relative">
                                <button
                                    type="button"
                                    @click="open = !open"
                                    class="flex items-center gap-2 rounded-full border border-[color:var(--cp-border)] bg-white/90 px-2.5 py-2 shadow-sm transition hover:border-[color:var(--cp-border-strong)]"
                                >
                                    ...
                                    <span class="hidden xl:block max-w-[9rem] truncate text-sm font-bold text-[color:var(--cp-plum-950)]">{{ $userName }}</span>
                                ...
                                <div
                                    ...
                                    @click.outside="open = false"
                                    x-transition
                                    class="absolute right-0 mt-3 w-64 rounded-[1.5rem] border border-[color:var(--cp-border)] bg-white p-2 shadow-2xl"
                                >
                    ...
                    <div class="flex items-center gap-2 lg:hidden">
                        <a href="{{ route('contact') }}" class="cp-icon-button" aria-label="Contacter un conseiller" title="Contacter un conseiller">
                            <i class="fa-solid fa-headset text-sm"></i>
                        </a>
    ...
    <div
        ...
        class="fixed inset-0 top-[5.75rem] bg-[#1f1230]/36 backdrop-blur-sm lg:hidden"
    ></div>
    ...
    <div class="cp-shell lg:hidden">
        <div
            ...
            class="relative mt-3 max-h-[calc(100vh-6.75rem)] overflow-y-auto rounded-[1.75rem] border border-[color:var(--cp-border)] bg-[color:var(--cp-panel-strong)] p-4 shadow-2xl"
        >
            <div class="rounded-[1.35rem] border border-[color:var(--cp-border)] bg-[#f8f4ff] px-4 py-4">
                <p class="text-xs font-black uppercase tracking-[0.2em] text-[color:var(--cp-plum-800)]">Navigation claire</p>
                <p class="mt-2 text-sm leading-6 text-[color:var(--cp-ink-soft)]">Choisis directement le service voulu. Le reste est dans le footer ou l’aide.</p>
            </div>
            ...
            <nav class="mt-4 space-y-2">
                @foreach($mainNavigation as $item)
                    ...
                    <a
                        ...
                        @click="mobileMenuOpen = false"
                        @class([
                            'flex items-center justify-between rounded-[1.15rem] border px-4 py-3 text-sm font-bold transition',
                    ...
                    >
                        <span class="flex items-center gap-3">
                            <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#efe6ff] text-[color:var(--cp-plum-800)]">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
                @foreach($secondaryNavigation as $item)
                    <a href="{{ route($item['route']) }}" @click="mobileMenuOpen = false" class="cp-secondary-button !w-full !rounded-[1rem] !px-4 !py-3 text-sm">
            ...
            <div class="mt-4 grid grid-cols-2 gap-2">
                <a href="{{ $mobileLink }}" class="cp-primary-button !rounded-[1rem] !px-4 !py-3 text-sm">
                    <i class="fa-solid fa-phone text-sm"></i>
                    <span>Appeler</span>
                </a>
                <a href="{{ $whatsAppUrl }}" target="_blank" rel="noopener noreferrer" class="cp-secondary-button !rounded-[1rem] !px-4 !py-3 text-sm">
                    <i class="fa-brands fa-whatsapp text-sm"></i>
                    <span>WhatsApp</span>
                </a>
            </div>

            <div class="mt-4 rounded-[1.35rem] border border-[color:var(--cp-border)] bg-white p-4">
                <div>
                ...
                <button id="theme-toggle-mobile" type="button" class="mt-4 flex w-full items-center justify-between gap-4 rounded-[1rem] bg-[#f6f0ff] px-4 py-3 text-left text-sm font-black text-[color:var(--cp-plum-900)]">
            ...
            <div class="mt-4 rounded-[1.2rem] bg-[#1e112c] px-4 py-4 text-white">

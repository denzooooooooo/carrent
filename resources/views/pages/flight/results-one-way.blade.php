...
@section('title', 'Résultats - Aller Simple - Carré Premium')

@section('content')
    <div class="bg-gradient-to-br from-purple-600 to-purple-700 text-white py-8">
        <div class="container mx-auto px-4 text-center">
            <h1 class="text-3xl lg:text-4xl font-black mb-2">✈️ Vols Aller Simple</h1>
                ...
                <form method="POST" action="{{ route('flights.search') }}" class="flex flex-wrap items-end gap-3">
                    ...
                    <div class="flex-1 min-w-[100px]">
                        ...
                        <select name="adults"
                            class="w-full px-3 py-2 border-2 border-gray-300 rounded-lg text-sm font-semibold">
                            @for($i = 1; $i <= 9; $i++)
                                <option value="{{ $i }}" {{ ($searchParams['adults'] ?? 1) == $i ? 'selected' : '' }}>{{ $i }}
                                </option>
                            @endfor
                        </select>
                    </div>
                    ...
                    <div class="flex-1 min-w-[130px]">
                        ...
                        <select name="travel_class"
                            ...
                            <option value="FIRST" {{ ($searchParams['travel_class'] ?? '') == 'FIRST' ? 'selected' : '' }}>
                                Première</option>
                        </select>
                    </div>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="lg:w-1/4">
                    <div class="bg-white rounded-2xl shadow-2xl p-6 sticky top-24 border border-purple-100">
                        ...
                        {{-- Compteur --}}
                        <div
                            class="mb-6 p-4 bg-gradient-to-r from-purple-50 to-amber-50 rounded-xl border border-purple-200">
                            <span class="text-sm font-black text-purple-700" id="resultsCount">
                                {{ count($results['best_flights'] ?? []) + count($results['other_flights'] ?? []) }} vols
                                trouvés
                            </span>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div
                                class="p-4 bg-gradient-to-r from-purple-50 to-amber-50 rounded-xl border border-purple-200">
                                @php
                        ...
                        <div class="mb-6">
                            ...
                            <div
                                ...
                                <div class="flex justify-between text-sm font-bold text-purple-700 mb-2">
                                    ...
                                    <span id="durationValue">{{ floor($maxDuration / 60) }}h
                                        {{ $maxDuration % 60 }}min</span>
                                </div>
                    ...
                    <div id="searchResults">
                        @if(!empty($results['best_flights']))
                            <div class="mb-8" id="bestFlights">
                                ...
                                <div class="space-y-4">
                                    @foreach($results['best_flights'] as $flight)
                                        <x-one-way-flight-card :flight="$flight" :searchParams="$searchParams" :isBest="true" />
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        @if(!empty($results['other_flights']))
                            <div id="otherFlights">
                                ...
                                <div class="space-y-4">
                                    @foreach($results['other_flights'] as $flight)
                                        <x-one-way-flight-card :flight="$flight" :searchParams="$searchParams" :isBest="false" />
                                    @endforeach
                                </div>
                            </div>
                        @endif
